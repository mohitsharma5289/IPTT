...
Runs against the project conftest seeds, like the API smoke tests.
"""
...
@pytest.fixture
def workbook_bytes(db, seeded) -> bytes:
    from app.services.excel import build_workbook

    return build_workbook(db, seeded.project_id)
...
def _parse(db, project_id: int, content: bytes, dry_run=True):
...
def test_export_covers_every_node(db, workbook_bytes, seeded):
    from app.models import Scope
    from sqlalchemy import func, select

    expected = db.scalar(
        select(func.count()).select_from(Scope).where(Scope.project_id == seeded.project_id)
    )
    wb = _load(workbook_bytes)
    assert wb["Execution"].max_row - 1 == expected


def test_export_can_be_filtered_to_one_circle(db, seeded):
    from app.services.excel import build_workbook

    wb = _load(build_workbook(db, seeded.project_id, circle=seeded.circles[1]))
    circles = {row[1] for row in wb["Execution"].iter_rows(min_row=2, values_only=True)}
    assert circles == {seeded.circles[1]}


# --- round trip -------------------------------------------------------------


def test_unmodified_export_reimports_as_a_no_op(db, workbook_bytes, seeded):
    report = _parse(db, seeded.project_id, workbook_bytes)
    assert report.ok, report.errors
    assert report.changes == []
    assert report.nodes_matched > 0


def test_an_edit_is_detected_and_described(db, workbook_bytes, seeded):
    wb = _load(workbook_bytes)
    ws = wb["Execution"]
    ...
    report = _parse(db, seeded.project_id, _save(wb))
    assert report.ok, report.errors
    assert report.changes
...
def test_reordered_columns_still_import_correctly(db, workbook_bytes, seeded):
    ...
    report = _parse(db, seeded.project_id, _save(reordered))
    assert report.ok, report.errors
...
, seeded):
    ...
    report = _parse(db, seeded.project_id, _save(trimmed))
    assert report.ok, report.errors
    assert report.changes == []
    assert report.nodes_matched > 0


def test_removing_rows_narrows_the_import(db, workbook_bytes, seeded):
    wb = _load(workbook_bytes)
    ws = wb["Execution"]
    rows = [list(r) for r in ws.iter_rows(values_only=True)]
    ...
    report = _parse(db, seeded.project_id, _save(subset))
    assert report.ok, report.errors
...
def test_a_mangled_activity_heading_is_rejected_not_guessed(db, workbook_bytes, seeded):
    wb = _load(workbook_bytes)
    ws = wb["Execution"]
    header = [str(c.value) for c in ws[1]]
    start_col = next(i for i, h in enumerate(header, 1) if h.endswith("— Start"))
...
def test_a_missing_node_column_is_rejected(db, workbook_bytes, seeded):
    wb = _load(workbook_bytes)
    ws = wb["Execution"]
...
def test_a_finish_without_a_start_is_rejected(db, workbook_bytes, seeded):
    wb = _load(workbook_bytes)
    ws = wb["Execution"]
    header = [str(c.value) for c in ws[1]]
    start_col = next(i for i, h in enumerate(header, 1) if h.endswith("— Start"))
...
def test_a_finish_before_its_start_is_rejected(db, workbook_bytes, seeded):
...
def test_an_unparseable_date_is_reported_with_its_location(db, workbook_bytes, seeded):
    wb = _load(workbook_bytes)
    ws = wb["Execution"]
    header = [str(c.value) for c in ws[1]]
    start_col = next(i for i, h in enumerate(header, 1) if h.endswith("— Start"))
...
def test_unknown_nodes_are_warned_about_not_silently_dropped(db, workbook_bytes, seeded):
    wb = _load(workbook_bytes)
    ws = wb["Execution"]
...
def test_a_sheet_with_no_execution_tab_is_rejected(db, seeded):
    wb = Workbook()
...
def test_a_non_workbook_is_rejected_cleanly(db, seeded):
    report = _parse(db, seeded.project_id, b"this is not a spreadsheet")
    assert not report.ok
    assert any("could not be opened" in e for e in report.errors)


def test_plan_columns_are_ignored_even_if_edited(db, workbook_bytes, seeded):
...
def test_dry_run_writes_nothing(db, workbook_bytes, seeded):
    ...
    report = _parse(db, seeded.project_id, _save(wb), dry_run=True)
...
def test_applying_writes_values_audit_rows_and_recomputed_delay(db, workbook_bytes, seeded):
    from sqlalchemy import func, select
    ...
    report = _parse(db, seeded.project_id, _save(wb), dry_run=False)
    assert report.ok, report.errors
    db.flush()
...
def test_a_file_with_any_error_applies_nothing(db, workbook_bytes, seeded):
